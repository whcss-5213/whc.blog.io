...
| 方案 | 优点 | 缺点 | 适用场景 |
| :--- | :--- | :--- | :--- |
| **EventSource** | 原生支持、配置简单、自动重连 | 仅支持 GET、无自定义头、不支持 POST | 简单单向推送（如通知、聊天） |
| **fetch + eventsource-parser** | 支持自定义头/POST、流式解析、内存可控 | 需手动处理重连 | 复杂场景、鉴权、跨域 |
| **axios** | 兼容旧项目、统一请求库 | 不支持流式解析、内存占用大 | 老项目迁移、必须使用 axios |

**推荐优先级**：原生 EventSource ＞ fetch + eventsource-parser ＞ axios
